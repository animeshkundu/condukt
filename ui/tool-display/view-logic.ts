...
export function filterEvents(events: readonly IAgentDebugEvent[], filter: IAgentDebugEventFilter): IAgentDebugEvent[] {
  return events.filter(e => {
    if (filter.categories && filter.categories.length > 0) {
      if (!filter.categories.includes(e.category)) {
        return false;
      }
    }
...
const CATEGORY_ICONS: Record<AgentDebugEventCategory, string> = {
  discovery: 'search',
  toolCall: 'tools',
  llmRequest: 'cloud',
  error: 'error',
...
export function getEventIcon(event: IAgentDebugEvent): string {
  return CATEGORY_ICONS[event.category] ?? 'circle';
}
...
export function getEventStatusClass(event: IAgentDebugEvent): string {
  switch (event.category) {
    ...
    case 'toolCall': {
      ...
      if (tc.status === 'failure') { return 'status-error'; }
      if (tc.status === 'pending') { return 'status-warning'; }
      return 'status-success';
    }
    case 'llmRequest': {
      const lr = event as ILLMRequestEvent;
      if (lr.status === 'failure') { return 'status-error'; }
      return 'status-success';
    }
    case 'loopControl': {
      const lc = event as ILoopControlEvent;
      if (lc.loopAction === 'stop' && lc.reason) { return 'status-warning'; }
      return 'status-info';
    }
    case 'discovery':
      return 'status-info';
...
export function formatEventDetail(event: IAgentDebugEvent): Record<string, string> {
  ...
  switch (event.category) {
    case 'toolCall': {
      ...
      if (tc.subAgentName) { result['SubAgent'] = tc.subAgentName; }
      result['Args'] = tc.argsSummary;
      if (tc.durationMs !== undefined) { result['Duration'] = `${tc.durationMs}ms`; }
      if (tc.resultSummary) { result['Result'] = tc.resultSummary; }
      if (tc.errorMessage) { result['Error'] = tc.errorMessage; }
      break;
    }
    case 'llmRequest': {
      const lr = event as ILLMRequestEvent;
      ...
      if (lr.errorMessage) { result['Error'] = lr.errorMessage; }
      break;
    }
    case 'error': {
      const ee = event as IErrorEvent;
      result['Type'] = ee.errorType;
      if (ee.originalError) { result['Error'] = ee.originalError; }
      if (ee.toolName) { result['Tool'] = ee.toolName; }
      break;
    }
    case 'loopControl': {
      const lc = event as ILoopControlEvent;
      result['Action'] = lc.loopAction;
      if (lc.iterationIndex !== undefined) { result['Iteration'] = String(lc.iterationIndex); }
      if (lc.reason) { result['Reason'] = lc.reason; }
      break;
    }
    case 'discovery': {
...
export function computeSessionSummary(events: readonly IAgentDebugEvent[]): ISessionSummary {
  ...
  return { toolCount, totalTokens, durationMs, errorCount, cachedTokenRatio };
}

// ── Formatting ───────────────────────────────────────────────────────────────

const CATEGORY_LABELS: Record<AgentDebugEventCategory, string> = {
  discovery: 'Discovery',
  toolCall: 'Tool Call',

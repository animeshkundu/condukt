 ...
 *
 * SdkBackend uses a closed explicit classifier with data checks to account
 * for heartbeat forward progress. Classification remains responsible for
 * event semantics and unknown-event diagnostics. Keep the informational shim
 * for SubprocessBackend's JSONL warning suppression.
 */
...
const FORWARD_PROGRESS = new Set([
  'model.call_start',
  ...
  'assistant.usage', 'model.call_finished',
...
const INFORMATIONAL = new Set([
  ...
  'assistant.message_start', 'assistant.idle', 'tool.user_requested',
  ...
  'assistant.fusion_phase_failed', 'prompt_cache_break',
...
export function classifySdkEvent(type: string): SdkEventClass | undefined {
  ...
  if (FORWARD_PROGRESS.has(type)) return 'streaming-liveness';
  if (INFORMATIONAL.has(type)) return 'informational';
  return undefined;
}

/**
 * Returns true only when an SDK event proves novel forward progress.
 * Unknown and informational events are transport activity, not heartbeat evidence.
 */
export function isSdkForwardProgress(
  type: string,
  data: Readonly<Record<string, unknown>> | undefined,
): boolean {
  if (!FORWARD_PROGRESS.has(type)) return false;

  switch (type) {
    case 'assistant.message_delta':
    case 'assistant.reasoning_delta':
      return typeof data?.deltaContent === 'string' && data.deltaContent.length > 0;
    ...
    case 'assistant.tool_call_delta':
      return typeof data?.inputDelta === 'string' && data.inputDelta.length > 0;
    case 'assistant.streaming_delta':
      return typeof data?.totalResponseSizeBytes === 'number'
        && Number.isFinite(data.totalResponseSizeBytes)
        && data.totalResponseSizeBytes >= 0;
    case 'tool.execution_partial_result':
      return typeof data?.partialOutput === 'string' && data.partialOutput.length > 0;
    case 'tool.execution_progress':
      return typeof data?.progressMessage === 'string' && data.progressMessage.length > 0;
    case 'assistant.usage':
      return typeof data?.apiCallId === 'string' && data.apiCallId.length > 0;
    case 'assistant.intent':
      return typeof data?.intent === 'string' && data.intent.length > 0;
    default:
      return true;
  }
}
